...
//
homeApp.controller('homeData', function($scope,$http) {
  ...
  $http({
    ...
    url: '/search/options'
      ...
      $scope.search_options = response.data;
  ...
  $http({
    ...
    url: '/article/selected'
      ...
      $scope.slide_content = response.data;
  }, function errorCallback(response) {
      // 请求失败执行代码
  });
  ...
  $http({
    ...
    url: '/product/recent'
      ...
      $scope.recent_browse = response.data;
  }, function errorCallback(response) {
      // 请求失败执行代码
  });
